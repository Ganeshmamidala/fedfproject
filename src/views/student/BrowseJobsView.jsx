...
const BrowseJobsView = () => {
  ...
  const filteredJobs = mockJobs.filter(job => {
    ...
    // Parse salary from the salary_range string (e.g., "$80,000 - $120,000")
    const matchesSalary = (() => {
      if (!salaryRange.min && !salaryRange.max) return true;
      
      const salaryMatch = job.salary_range.match(/\$?(\d+,?\d*)/);
      if (salaryMatch) {
        const jobSalary = parseInt(salaryMatch[1].replace(/,/g, ''));
        if (salaryRange.min && jobSalary < salaryRange.min) return false;
        if (salaryRange.max && jobSalary > salaryRange.max) return false;
      }
      return true;
    })();

    return matchesSearch && matchesLocation && matchesJobType && matchesCompany && matchesSalary;
  });
  ...
  return (
    <div className="p-6">
      ...
      <JobFilters
        ...
        setLocationFilter={setSelectedLocation}
        jobTypeFilter={selectedJobType}
        setJobTypeFilter={setSelectedJobType}
        companyFilter={selectedCompany}
        setCompanyFilter={setSelectedCompany}
